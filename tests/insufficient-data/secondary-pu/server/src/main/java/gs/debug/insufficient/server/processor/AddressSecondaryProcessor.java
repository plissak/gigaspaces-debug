...
public class AddressSecondaryProcessor implements InitializingBean, DisposableBean {
...
//			}
	...
	public void readAll() {
		...
		for (Address address : readAccess.getAddresses()) {
	...
	private void processBatch(BatchRemoteEvent batchEvent) {
		for (RemoteEvent event : batchEvent.getEvents()) {
			...
			try {
				process((Address)entryEvent.getObject(), isDelete);
			}
			catch (Exception e) {
	...
	private void processException(ListenerExecutionFailedException exception, Address data, Object source) {
	...
	protected synchronized void createContainer() {
		...
		container.setTemplate(new Address());
		container.setFifo(true);
		container.setNotifyAll(true);

		container.setExceptionHandler(new EventExceptionHandler<Address>() {
			@Override
			public void onException(ListenerExecutionFailedException exception, Address data, GigaSpace space, TransactionStatus txStatus,
					Object source) throws RuntimeException {
				processException(exception, data, source);
			}

			@Override
			public void onSuccess(Address data, GigaSpace space, TransactionStatus txStatus, Object source) throws RuntimeException {}
		});
